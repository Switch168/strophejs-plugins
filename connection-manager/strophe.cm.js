...
/*jslint browser: true, undef: true, bitwise: true, regexp: true, newcap: true, immed: true */ 
/*global Strophe, $iq*/
...
Strophe.addConnectionPlugin('connectionmanager', {
    ...
    enable: function(){
        ...
        conn._queueData = function (element) {
            if(that._enqueueElement(element)){
                ...
                //DF.console.log("NOT passing through queue call to Strophe Connection!");
            }
        };
        
        // hook into _onRequestStateChange event
    ...
    disable: function(){
        ...
        // TOOD restore old prototype
        // this.conn._onRequestStateChange = Strophe.Bosh.prototype._onRequestStateChange;
...
//         DF.console.info("Strophe connection status:", this._status_lookup[status] || status,
//                         condition);
...
//         DF.console.info("req sent:", el);
        ...
        if( (Strophe.isTagEqual(el, "iq") &&
             el.getAttribute("id").indexOf("_auth_") != -1)
...
//         DF.console.info("sentQueue:", this.element_queue.length, this.element_queue);
//         DF.console.info("_requests:", this.conn._requests.length, this.conn._requests);
...
//         DF.console.info("req received:", req);
